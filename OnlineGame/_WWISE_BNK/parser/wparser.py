...
from . import wdefs, wio, wmodel
from . import wparser_cls as wcls
from . import wparser_plg as wplg
...
def CAkBankMgr__LoadSource(obj, cls, subnode=False):
    ...
    if cls.version <= 46:
        ...
        if cls.version <= 26:  # 26=TH
            ...
            elem.u32("uSampleRate")
            elem.U32("uFormatBits").bit("uChannelMask", elem.lastval, 0, 0x3FFFF).bit("uBitsPerSample", elem.lastval, 18, 0x3F).bit("uBlockAlign", elem.lastval, 24, 0x1F).bit("uTypeID", elem.lastval, 29, 0x3).bit("uInterleaveID", elem.lastval, 31)
    else:
        pass
    ...
    if cls.version <= 26:
        ...
        elem.U8x("uSourceBits").bit("bIsLanguageSpecific", elem.lastval, 0).bit("bHasSource", elem.lastval, 1).bit("bExternallySupplied", elem.lastval, 2)
    else:
        elem.U8x("uSourceBits").bit("bIsLanguageSpecific", elem.lastval, 0).bit("bPrefetch", elem.lastval, 1).bit("bNonCachable", elem.lastval, 3).bit("bHasSource", elem.lastval, 7)
...
def CAkParameterNode__SetAdvSettingsParams(obj, cls):
    ...
    if cls.version <= 36:  # 36=UFC
        ...
        obj.U8x("byBitVector").bit("bKillNewest", obj.lastval, 0).bit("bUseVirtualBehavior", obj.lastval, 1).bit("bIgnoreParentMaxNumInst", obj.lastval, 3).bit("bIsVVoicesOptOverrideParent", obj.lastval, 4)
        obj.U8x("eVirtualQueueBehavior").fmt(wdefs.AkVirtualQueueBehavior)
        obj.u16("u16MaxNumInstance")
        obj.U8x("eBelowThresholdBehavior").fmt(wdefs.AkBelowThresholdBehavior)
        obj.U8x("byBitVector").bit("bOverrideHdrEnvelope", obj.lastval, 0).bit("bOverrideAnalysis", obj.lastval, 1).bit("bNormalizeLoudness", obj.lastval, 2).bit("bEnableEnvelope", obj.lastval, 3)
    ...
    if count > 0:
        ...
        for elem in obj.list("pFXChunk", "FXChunk", count):
            ...
            if cls.version <= 26:
                ...
                elem.U8x("bitVector").bit("bBypass", obj.lastval, 0).bit("bShareSet", obj.lastval, 1).bit("bRendered", obj.lastval, 2)
...
def CAkParameterNodeBase__SetAuxParams(obj, cls):
    ...
    if cls.version <= 89:
        ...
        obj.U8x("byBitVector").bit("bOverrideUserAuxSends", obj.lastval, 2).bit("bHasAux", obj.lastval, 3).bit("bOverrideReflectionsAuxBus", obj.lastval, 4)  # bHasAux in v122, > 135
...
def CAkParameterNodeBase__SetNodeBaseParams(obj, cls):
    ...
    if cls.version <= 56:
        ...
        obj.U8x("byBitVector").bit("bPriorityOverrideParent", obj.lastval, 0).bit("bPriorityApplyDistFactor", obj.lastval, 1).bit("bOverrideMidiEventsBehavior", obj.lastval, 2).bit("bOverrideMidiNoteTracking", obj.lastval, 3).bit("bEnableMidiNoteTracking", obj.lastval, 4).bit(
            "bIsMidiBreakLoopOnNoteOff", obj.lastval, 5
        )
    ...
    if count > 0:
        ...
        for elem in obj.list("pFXChunk", "FXChunk", count):
            ...
            if cls.version <= 145:
                ...
                elem.U8x("bitVector").bit("bBypass", obj.lastval, 0).bit("bShareSet", obj.lastval, 1)
...
def CAkActionPause__SetActionSpecificParams(obj, cls):
    ...
    if cls.version <= 56:
        ...
        obj.U8x("byBitVector").bit("bIncludePendingResume", obj.lastval, 0).bit("bApplyToStateTransitions", obj.lastval, 1).bit("bApplyToDynamicSequence", obj.lastval, 2)
    return
...
def CAkActionResume__SetActionSpecificParams(obj, cls):
    ...
    if cls.version <= 56:
        ...
        obj.U8x("byBitVector").bit("bIsMasterResume", obj.lastval, 0).bit("bApplyToStateTransitions", obj.lastval, 1).bit("bApplyToDynamicSequence", obj.lastval, 2)
    return
...
def CAkActionStop__SetActionSpecificParams(obj, cls):
    ...
    obj.U8x("byBitVector").bit("bApplyToStateTransitions", obj.lastval, 1).bit("bApplyToDynamicSequence", obj.lastval, 2)
    return
...
def CAkActionActive__SetActionParams(obj, cls):
    ...
    obj.U8x("byBitVector").bit("eFadeCurve", obj.lastval, 0, mask=0x1F, fmt=wdefs.AkCurveInterpolation)
...
def CAkAction__SetInitialValues(obj, cls):
    ...
    if cls.version <= 56:
        ...
        if cls.version <= 65:  # 65=DmC
            ...
            obj.U8x("idExt_4").bit("bIsBus", obj.lastval, 0)
...
def CAkRanSeqCntr__SetInitialValues(obj, cls):
    ...
    if cls.version <= 89:
        ...
        obj.U8x("byBitVector").bit("_bIsUsingWeight", obj.lastval, 0).bit("bResetPlayListAtEachPlay", obj.lastval, 1).bit("bIsRestartBackward", obj.lastval, 2).bit("bIsContinuous", obj.lastval, 3).bit("bIsGlobal", obj.lastval, 4)
...
def CAkSwitchCntr__SetInitialValues(obj, cls):
    ...
    for elem in obj.list("rParams", "AkSwitchNodeParams", obj.lastval):
        ...
        if cls.version <= 89:
            ...
            elem.U8x("byBitVector").bit("bIsFirstOnly", elem.lastval, 0).bit("bContinuePlayback", elem.lastval, 1)
            elem.U8x("byBitVector").bit("eOnSwitchMode", elem.lastval, 0, 0x7, fmt=wdefs.AkOnSwitchMode)
        else:
            elem.U8x("byBitVector").bit("bIsFirstOnly", elem.lastval, 0).bit("bContinuePlayback", elem.lastval, 1)
...
def CAkBus__SetInitialParams(obj, cls):
    ...
    if cls.version <= 53:
        ...
        obj.U8x("byBitVector").bit("bMainOutputHierarchy", obj.lastval, 0).bit("bIsBackgroundMusic", obj.lastval, 1)

        obj.U8x("byBitVector").bit("bKillNewest", obj.lastval, 0).bit("bUseVirtualBehavior", obj.lastval, 1)

        obj.U16("u16MaxNumInstance")
        obj.U32("uChannelConfig").bit("uNumChannels", obj.lastval, 0, 0xFF).bit("eConfigType", obj.lastval, 8, 0xF, fmt=wdefs.AkChannelConfigType).bit("uChannelMask", obj.lastval, 12, 0xFFFFF, fmt=wdefs.fmt_ch)

        obj.U8x("byBitVector").bit("bIsHdrBus", obj.lastval, 0).bit("bHdrReleaseModeExponential", obj.lastval, 1)

    else:
        obj.U8x("byBitVector").bit("bKillNewest", obj.lastval, 0).bit("bUseVirtualBehavior", obj.lastval, 1).bit("bIsMaxNumInstIgnoreParent", obj.lastval, 2).bit("bIsBackgroundMusic", obj.lastval, 3)
...
def CAkMusicNode__SetMusicNodeParams(obj, cls):
    ...
    if cls.version <= 89:
        ...
        obj.U8x("uFlags").bit("bOverrideParentMidiTempo", obj.lastval, 1).bit("bOverrideParentMidiTarget", obj.lastval, 2).bit("bMidiTargetTypeBus", obj.lastval, 3)
...
def CAkMusicTrack__SetInitialValues(obj, cls):
    ...
    if cls.version <= 89:
        ...
        obj.U8x("uOverrides").bit("bOverrideParentMidiTempo", obj.lastval, 1).bit("bOverrideParentMidiTarget", obj.lastval, 2)
...
def CAkBankMgr__ProcessBankHeader(obj):
    ...
    if version <= 26:
        ...
        obj.U32("uAltValues").bit("bUnused", obj.lastval, 0, 0xFFFF).bit("bDeviceAllocated", obj.lastval, 16, 0xFFFF)
    else:
        obj.U32("uAltValues").bit("uAlignment", obj.lastval, 0, 0xFFFF).bit("bDeviceAllocated", obj.lastval, 16, 0xFFFF)
...
class Parser(object):
    ...
    def parse_banks(self, filenames):
        loaded_filenames = []
        for filename in tqdm(filenames, ncols=150):
