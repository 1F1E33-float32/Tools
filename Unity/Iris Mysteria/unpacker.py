...
def exportTextAsset(obj, fp, extension=".txt"):
...
def extract_assets(source, target, include_types=None, ignore_first_dirs=0, append_path_id=False):
    ...
    for obj_path, obj in tqdm(sorted_items, ncols=150):
        if include_types and obj.type.name not in include_types:
            continue
        if obj_path.endswith("default_chars.txt"):
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src", default=r"D:\Dataset_Game\あいりすミスティリア！ 〜少女のつむぐ夢の秘跡〜\DEC\assetbundle\scenarios")
    parser.add_argument("--dst", default=r"D:\Fuck_galgame\script")
    parser.add_argument("--id", default=True)
    parser.add_argument("--ignore", type=int, default=4, metavar="N")
