    ...
    for entry in tqdm(banks_dicts, ncols=150):
        for name, bd in entry.items():
            ...
            if media_index == {}:
                continue
            bank_info[name] = {
                "dwChunkSize": media_index["dwChunkSize"],
                "events": build_event_media_map(bd)
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bnk_root", 
        default=Path(r"D:\Dataset_Game\com.bluepoch.m.en.reverse1999\Android\audios\Android"),
        help="Root folder containing language subfolders with .bnk files"
    )
    parser.add_argument(
        "--out_root",   
        default=Path(r"D:\Dataset_Game\com.bluepoch.m.en.reverse1999\EXP\audios\Android"),
        help="Output root for extracted WEMs organized by language"
    )
