...
def parse_args(args=None, namespace=None):
    ...
    parser.add_argument("--root", type=str, default=r"D:/Dataset_Game/WutheringWaves/Client/Content/Aki")
    parser.add_argument("--out_dir", type=str, default=r"D:\Dataset_Game\WutheringWaves\EXP")
...
_AUDIO_RE = re.compile(r'^(?P<lang>zh|ja|en|ko)_vo_(?P<idx>[^.]+)\.wav$', re.I)
def build_flow_audio_map(root):
    ex_base = os.path.join(root, "WwiseAudio_Generated", "WwiseExternalSource")

    mapping = {}
    for root, _, files in tqdm(os.walk(ex_base), ncols=150, desc=f"flow_audio_map"):
        for fname in files:
            ...
            idx  = m.group('idx')
            key  = f"{lang}_{idx}"
            mapping[key] = os.path.join(root, fname)  # 或者直接 fname
    return mapping
...
def build_favor_audio_map(root):
    ...
    for lang in ("zh", "ja", "en", "ko"):
        ...
        for fname in tqdm(os.listdir(ev_lang_dir), ncols=150, desc=f"{lang}_favor_audio_map"):
            ...
            if len(ids) == 1:
                wem = os.path.join(media_base, lang, f"{ids[0]}.wav")
...
def process_flowstate(cursor_flowstate):
    ...
    result = []
    for bin_data in tqdm(bin_datas, ncols=150, desc=f"flowstate"):
...
def process_favorword(cursor_favorword):
    ...
    res = []
    for row in tqdm(rows, ncols=150, desc=f"favorword"):
