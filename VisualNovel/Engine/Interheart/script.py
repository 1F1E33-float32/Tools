...
def parse_args(args=None, namespace=None):
    ...
    p.add_argument("-VO", type=str, default=r"D:\Fuck_VN\voice")
...
def process_script(lines, results, results_char):
    ...
    while i < n:
        ...
        if len(parts) == 3:
            raw_voice = re.compile(r"[（(]\s*([^）)]+?)\s*[）)]").search(parts[1])
            if not raw_voice:
                i += 1
                continue
            raw_voice = raw_voice.group(1)
        ...
        results.append({"Speaker": speaker, "Folder": folder, "Voice": voice, "Text": text})
        i = j + 1
