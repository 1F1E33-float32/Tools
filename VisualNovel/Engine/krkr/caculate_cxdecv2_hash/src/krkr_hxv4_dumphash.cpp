...
HMODULE LoadLibraryW_hook(LPCWSTR name)
{
    ...
    LOGi("LoadLibraryW name=%ls hmod=%p\n", name, hmod);
    ...
    if(wcsstr(name, L"krkr_"))
    {
        ...
        if(addr)
        {
            MH_CreateHook(addr, reinterpret_cast<LPVOID>(V2Link_hook), reinterpret_cast<LPVOID*>(&V2Link_org));
        ...
        if(addr)
        {
            MH_CreateHook(addr, reinterpret_cast<LPVOID>(CreateHxv4CompoundStorageMedia_hook), reinterpret_cast<LPVOID*>(&CreateHxv4CompoundStorageMedia_org));
...
static void init()
{
    ...
    status = MH_CreateHook(reinterpret_cast<LPVOID*>(LoadLibraryW), reinterpret_cast<LPVOID*>(LoadLibraryW_hook), reinterpret_cast<LPVOID*>(&LoadlibraryW_org));
    ...
    LOGi("MH_EnableHook LoadLibraryW success\n");
}
