...
def process_type1_0(lines, results):
    """
    @nm t="臨" s=rin_0015
    「んー、特には何も。[r]
    そっちが寂しいかなーって思ってさ」[np]
    ...
    def handle_saudio(match, line_index):
        ...
        results.append({"Speaker": speaker, "Voice": voice1, "Text": text1})

        # 添加第二个条目 (back)
        voice2 = match.group("voice2")
        text2 = text_cleaning_02(match.group("text2"))
        results.append({"Speaker": speaker, "Voice": voice2, "Text": text2})
...
def process_type1_2(lines, results):
    ...
    while i < n:
        ...
        if m_spk:
            ...
            text = text_cleaning_02("".join(tmp))
            results.append({"Speaker": speaker, "Voice": current_voice, "Text": text})
            continue
