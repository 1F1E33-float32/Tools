import struct
import sys
import pathlib

MjoKey = bytes([
    
])

MAGIC_IN  = b"MajiroObjX1.000\x00"
MAGIC_OUT = b"MajiroObjV1.000\x00"

def read_struct(fmt: str, data: bytes, offset: int):
    size = struct.calcsize(fmt)
    value = struct.unpack_from(fmt, data, offset)
    return (value[0] if len(value) == 1 else value), offset + size

def decrypt(buffer: bytearray, key: bytes) -> None:
    k_len = len(key)
    for i in range(len(buffer)):
        buffer[i] ^= key[i & (k_len - 1)]

def main(path_in: pathlib.Path):
    buf = path_in.read_bytes()
    if len(buf) < 16 or buf[:16] != MAGIC_IN:
        sys.exit("文件头不符合 MajiroObjX1.000 格式")

    off = 16                                             # 跳过 magic
    unknown0,  off = read_struct("<I", buf, off)
    unknown1,  off = read_struct("<I", buf, off)
    entries,   off = read_struct("<I", buf, off)

    entries_buf = buf[off : off + entries * 8]
    off += entries * 8

    data_length, off = read_struct("<I", buf, off)
    data_buf = bytearray(buf[off : off + data_length])  # 可变字节数组，方便就地解密

    decrypt(data_buf, MjoKey)
    path_out = path_in.with_suffix(path_in.suffix + ".dec")

    with path_out.open("wb") as fp:
        fp.write(MAGIC_OUT.ljust(16, b"\0"))             # 填充到 16 字节
        fp.write(struct.pack("<I", unknown0))
        fp.write(struct.pack("<I", unknown1))
        fp.write(struct.pack("<I", entries))
        fp.write(entries_buf)                            # <-- C++ 写错，这里写真正条目
        fp.write(struct.pack("<I", data_length))
        fp.write(data_buf)

    print("读取成功")
    print(f"Size of file: {len(buf)} bytes")
    print(f"Size of data_buffer: 0x{data_length:X}")
    print("解密完成")
    print(f"输出文件: {path_out}")

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python majiroutil.py <InputFile.mjo>")
        sys.exit(1)

    try:
        main(pathlib.Path(sys.argv[1]))
    except FileNotFoundError:
        sys.exit("找不到文件。")    